...
const VERIFY_TOKEN = process.env.WHATSAPP_VERIFY_TOKEN || "your_custom_secure_string_123";
...
// 2. Receiving Messages from Citizens via WhatsApp
app.post('/webhook', async (req, res) => {
    ...
    if (body.object) {
        if (body.entry && body.entry[0].changes && body.entry[0].changes[0].value.messages && body.entry[0].changes[0].value.messages[0]) {
            ...
            let from = body.entry[0].changes[0].value.messages[0].from; // Citizen's phone number
            let msgBody = body.entry[0].changes[0].value.messages[0].text?.body; // The message they sent

            if (msgBody) {
                console.log(`[WhatsApp Webhook] Received grievance from ${from}: ${msgBody}`);

                try {
                    // Create a grievance automatically from the WhatsApp message
                    // Using default coordinates for now (could be requested from user later via location message)
                    const lat = 28.6139;
                    const lng = 77.2090;
                    const regionId = findClosestRegion(lat, lng);

                    // Trigger AI Translation / Categorization
                    const aiResult = await analyzeGrievanceWithAI(msgBody, "Other", undefined, undefined);

                    const newId = `WA-${Math.floor(1000 + Math.random() * 9000)}`;
                    const newGrievance: Grievance = {
                      id: newId,
                    ...
                    serverGrievances.push(newGrievance);
                    recalculateAllScores();
                    console.log(`[WhatsApp Webhook] Successfully ingested grievance: ${newId}`);
                    
                    // TODO: Make an HTTP POST request back to Meta's API to send a confirmation reply to the citizen
                    // e.g. POST to https://graph.facebook.com/v17.0/${phoneNumberId}/messages with process.env.WHATSAPP_ACCESS_TOKEN
                } catch (err) {
